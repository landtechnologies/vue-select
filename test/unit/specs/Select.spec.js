...
describe('Select.vue', () => {
	...
	describe('When Groups Are Enabled', () => {
		it('should display group names as headings', () => {
			...
			const vm = new Vue({
				template: '<div><v-select ref="select" :options="options" groups taggable></v-select></div>',
				components: {vSelect},
				data: {
					options: [opt1, opt2]
				}
		...
		it('should display groups with items matching the filter', (done) => {
			...
			const vm = new Vue({
				template: '<div><v-select ref="select" :options="options" groups></v-select></div>',
				components: {vSelect},
				data: {
					options: [opt1, opt2, opt3]
				}
		...
		it('should select the correct item when using typeahead pointer', () => {
			...
			trigger(vm.$children[0].$refs.search, 'keyup', (e) => e.keyCode = 13)
			expect(vm.$children[0].mutableValue).toEqual(opt2);
		})
		...
		it('can add the current search text as the first item in the options list', () => {
			const vm = new Vue({
				template: '<div><v-select :options="options" :value="value" :multiple="true" groups taggable></v-select></div>',
				components: {vSelect},
				data: {
					value: ['one'],
					options: ['one', 'two']
